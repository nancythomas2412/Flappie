...
class MenuRenderer(
    ...
    fun draw(canvas: Canvas, bestScore: Int, bird: Bird, canPlay: Boolean = true, heartRefillTime: String? = null, totalCoins: Int = 0, currentLives: Int = 3, heartSprites: HeartSprites? = null) {
        ...
        // Best score
        canvas.drawText("Best: $bestScore", centerX, centerY + DensityUtils.dp(50f), smallTextPaint)

        // Draw bird above "Tap to Play" - responsive positioning
        val birdX = centerX
        val birdY = centerY - DensityUtils.dp(80f) // Responsive spacing instead of fixed pixels
        bird.x = birdX
        bird.y = birdY
        bird.draw(canvas)
        ...
        val premiumBuyLifeY = menuBuyLifeY + 150f  // Much more spacing to prevent overlap
    ...
    fun drawGameOver(
        ...
        // Scores and stats
        canvas.drawText("Score: $score", centerX, centerY + DensityUtils.dp(10f), smallTextPaint)
        canvas.drawText("Coins: $totalCoins", centerX, centerY + DensityUtils.dp(30f), smallTextPaint)
        canvas.drawText("Best: $bestScore", centerX, centerY + DensityUtils.dp(60f), smallTextPaint)
        ...
        if (canPlay) {
            if (currentLives > 0) {
                ...
                val continuePaint = Paint().apply {
                    ...
                    textSize = DensityUtils.dp(29f)
                    typeface = Typeface.DEFAULT_BOLD
                    textAlign = Paint.Align.CENTER
                    isAntiAlias = true
                }
                canvas.drawText("Tap to Continue", centerX, centerY + DensityUtils.dp(90f), continuePaint)
            } else {
                canvas.drawText("Tap to Restart", centerX, centerY + DensityUtils.dp(90f), smallTextPaint)
            }
        }
        ...
        // Premium Buy Life button below the coin button
        val premiumBuyLifeY = buyLifeY + 150f  // Much more spacing to prevent overlap
        drawPremiumBuyLifeButton(canvas, centerX, premiumBuyLifeY, currentLives == 0)
    }
    ...
    private fun drawBuyLifeButton(canvas: Canvas, centerX: Float, y: Float, totalCoins: Int) {
        // Responsive button sizing - reduced to prevent overlap
        val buttonWidth = DensityUtils.dp(180f) // Reduced width to prevent overlap
        val buttonHeight = DensityUtils.dp(40f) // Reduced height to prevent overlap  
        ...
        if (canAfford) {
            // Heart icon on left - responsive sizing
            val heartPaint = Paint().apply {
                textSize = DensityUtils.UI.getTextLarge() // dp(18f) - reduced from getTextTitle
                textAlign = Paint.Align.CENTER
                isAntiAlias = true
            }
            canvas.drawText("❤️", centerX - 150f, y - 2f, heartPaint)
            
            // Main text - centered and with better spacing
            val textPaint = Paint().apply {
                color = Color.WHITE
                textSize = DensityUtils.UI.getTextMedium() // dp(16f) - reduced from getTextLarge
                typeface = Typeface.DEFAULT_BOLD
                textAlign = Paint.Align.CENTER
                isAntiAlias = true
                setShadowLayer(2f, 1f, 1f, "#80000000".toColorInt())
            }
            canvas.drawText("Buy Life", centerX + 10f, y - 8f, textPaint)
            
            // Price - centered and with better spacing
            val pricePaint = Paint().apply {
                color = "#FFE0E0".toColorInt()
                textSize = DensityUtils.dp(12f) // 12dp - smaller than getTextSmall (14dp)
                typeface = Typeface.DEFAULT
                textAlign = Paint.Align.CENTER
                isAntiAlias = true
                setShadowLayer(1f, 1f, 1f, "#80000000".toColorInt())
            }
            canvas.drawText("500 coins", centerX + 10f, y + 40f, pricePaint)
            
        } else {
            // Grayed out version - responsive sizing
            ...
            canvas.drawText("💔", centerX - 150f, y - 2f, grayHeartPaint)
            ...
            canvas.drawText("Buy Life", centerX + 10f, y - 8f, grayTextPaint)
            ...
            canvas.drawText("Need 500 coins", centerX + 10f, y + 40f, needMorePaint)
        }
    }
    ...
    private fun drawPremiumBuyLifeButton(canvas: Canvas, centerX: Float, y: Float, enabled: Boolean = true) {
        // Responsive button sizing - reduced to prevent overlap (same as regular button)
        val buttonWidth = DensityUtils.dp(180f) // Reduced width to prevent overlap
        val buttonHeight = DensityUtils.dp(40f) // Reduced height to prevent overlap
        ...
        canvas.drawText("$3.00", centerX + 10f, y + 40f, pricePaint)
    }
}