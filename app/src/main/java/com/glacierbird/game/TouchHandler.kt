...
class TouchHandler(
    ...
    private val achievementRenderer: AchievementRenderer
) {
    ...
    fun handleTouch(
        ...
        android.util.Log.e("GLACIER_DEBUG", "🎮 HANDLE TOUCH: showStatistics=$showStatistics, showPause=$showPauseMenu, showSettings=$showSettings")
    ...
    private fun handlePauseMenuTouch(x: Float, y: Float): TouchCommand? {
        ...
        // Exit Game button area - exit to main menu
        val exitTextWidth = testPaint.measureText("Exit Game")
        ...
        if (x > centerX - exitButtonWidth/2f && x < centerX + exitButtonWidth/2f &&
            y > exitY && y < exitY + exitButtonHeight) {
            return TouchCommand.GoToMenu
        }
    ...
    private fun handleSettingsTouch(x: Float, y: Float): TouchCommand? {
        ...
        // Responsive audio button sizing - matches OverlayRenderer (optimized size)
        val soundButtonWidth = DensityUtils.dp(124f) // Match OverlayRenderer optimized size
        val soundButtonHeight = DensityUtils.dp(36f) // Match OverlayRenderer optimized size
        val closeButtonWidth = DensityUtils.dp(50f) // Match OverlayRenderer square size
        val closeButtonHeight = DensityUtils.dp(50f) // Match OverlayRenderer square size

        // Sound toggle button area - responsive sizing
        if (x > centerX - soundButtonWidth/2 && x < centerX + soundButtonWidth/2 &&
            y > centerY - DensityUtils.dp(30f) - soundButtonHeight/2 && y < centerY - DensityUtils.dp(30f) + soundButtonHeight/2) {
            return TouchCommand.ToggleSound
        }

        // Music toggle button area - responsive sizing
        val musicButtonY = centerY + DensityUtils.dp(60f) // Match OverlayRenderer responsive positioning
        if (x > centerX - soundButtonWidth/2 && x < centerX + soundButtonWidth/2 &&
            y > musicButtonY - soundButtonHeight/2 && y < musicButtonY + soundButtonHeight/2) {
            return TouchCommand.ToggleMusic
        }

        // Close settings button - responsive sizing
        val closeButtonY = centerY + DensityUtils.dp(160f) // Match OverlayRenderer responsive positioning
        if (x > centerX - closeButtonWidth/2 && x < centerX + closeButtonWidth/2 &&
            y > closeButtonY - closeButtonHeight/2 && y < closeButtonY + closeButtonHeight/2) {
    ...
    private fun handleShopTouch(x: Float, y: Float): TouchCommand? {
        ...
        // Responsive close button coordinates - match OverlayRenderer exactly
        val closeBtnSize = DensityUtils.dp(60f) // Match OverlayRenderer
        ...
        // Position close button at top-right of panel (match OverlayRenderer)
        ...
        // Responsive Buy Life button (first item in shop) - match OverlayRenderer
        val itemWidth = DensityUtils.dp(240f) // Match OverlayRenderer
        val itemHeight = DensityUtils.dp(50f) // Match OverlayRenderer
        val itemY = centerY + DensityUtils.dp(30f) // Match OverlayRenderer
    ...
    private fun handleAchievementsTouch(x: Float, y: Float): TouchCommand? {
        // Use AchievementRenderer's responsive close button area
        val closeButtonArea = achievementRenderer.getCloseButtonArea()
    ...
    private fun handleStatisticsTouch(x: Float, y: Float): TouchCommand? {
        ...
        android.util.Log.e("GLACIER_DEBUG", "🔥🔥🔥 STATISTICS TOUCH HANDLER 🔥🔥🔥")
        ...
        android.util.Log.e("GLACIER_DEBUG", "📐 TOUCH CALC: screen=${screenWidth}x$screenHeight, panel=${panelWidth}x$panelHeight at ($panelX,$panelY)")
        android.util.Log.e("GLACIER_DEBUG", "📐 TOUCH BUTTON: ($closeButtonX, $closeButtonY) size=${buttonSize}x$buttonSize")
        ...
        android.util.Log.e("GLACIER_DEBUG", "✅ ANY TOUCH CLOSES STATISTICS - Simple and user-friendly!")
    ...
    private fun handleBirdSkinsTouch(x: Float, y: Float): TouchCommand? {
        ...
        val closeButtonX = panelX + panelWidth - DensityUtils.dp(60f)
        val closeButtonY = panelY + DensityUtils.dp(10f)
        val closeButtonSize = DensityUtils.dp(50f)
    ...
    private fun handleMenuTouch(x: Float, y: Float, currentLives: Int = 0): TouchCommand? {
        ...
        // Buy Life button (matches MenuRenderer responsive sizing)
        val buttonWidth = DensityUtils.dp(180f) // Match MenuRenderer
        val buttonHeight = DensityUtils.dp(40f) // Match MenuRenderer
        ...
        val premiumBuyLifeY = menuBuyLifeY + 150f
    ...
    private fun handleGameOverTouch(x: Float, y: Float, currentLives: Int = 0): TouchCommand? {
        ...
        // Buy Life button (matches MenuRenderer responsive sizing) - always present
        val buttonWidth = DensityUtils.dp(180f) // Match MenuRenderer
        val buttonHeight = DensityUtils.dp(40f) // Match MenuRenderer
        ...
        val premiumBuyLifeY = buttonY + 150f
        val premiumButtonTop = premiumBuyLifeY - buttonHeight/2
        val premiumButtonBottom = premiumBuyLifeY + buttonHeight/2
