...
class HatRenderer(
    ...
    fun drawHatSelectionScreen(
        ...
        canvas.drawText("🎩 Hat Collection", screenWidth / 2f, panelY + 80f, titlePaint)
        ...
        val contentStartY = panelY + 180f     // More space for title/stats
        val contentWidth = panelWidth - 80f   // More side padding
        val contentX = panelX + 40f           // More side padding
    ...
    private fun drawHatGrid(
        ...
        val cardWidth = (gridWidth - 40f) / itemsPerRow // More generous spacing between cards
        val cardHeight = getHatCardHeight()  // Now uses responsive screen-based sizing
        val spacing = 30f      // More generous spacing for larger cards
    ...
    private fun drawHatCard(
        ...
        if (hatInfo.isUnlocked) {
            ...
            canvas.drawRoundRect(lockRect, 6f, 6f, lockOverlayPaint)
            ...
            canvas.drawText("🔒", spriteX, spriteY + 6f, smallLockPaint)
        }
    ...
    private fun drawCloseButton(canvas: Canvas, x: Float, y: Float) {
        ...
        canvas.drawText("✕", x + buttonSize/2f, y + buttonSize/2f + 15f, closeButtonTextPaint)
    }
    ...
    fun getHatCardAreas(hatCount: Int): List<RectF> {
        val panelWidth = screenWidth * 0.95f
        val panelX = (screenWidth - panelWidth) / 2f
        val contentWidth = panelWidth - 80f  // Match drawing layout
        val contentX = panelX + 40f  // Match drawing layout
        val contentStartY = (screenHeight - screenHeight * 0.90f) / 2f + 180f  // Match updated layout
        
        val itemsPerRow = 2
        val cardWidth = (contentWidth - 40f) / itemsPerRow // Match drawing layout spacing
        val cardHeight = getHatCardHeight()  // Match responsive card height
        val spacing = 30f      // Match drawing layout spacing
    ...
    fun drawUnlockMessage(canvas: Canvas, hatType: HatType, requiredScore: Int, currentScore: Int) {
        ...
        canvas.drawRoundRect(panelRect, 20f, 20f, borderPaint)
